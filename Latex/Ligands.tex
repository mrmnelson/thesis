...
In 2002 van Leeuwen \emph{et al.} reported their unsuccessful attempts to synthesise \tBuxantphos{} from either the dilithiated xanthene backbone or starting with 9,9-dimethyl-4,5-bis(dichlorophosphino)xanthene, suggesting that steric crowding from the presence of two \tBu{} groups on a single phosphorus atom prevented the successful coupling.\cite{Zuideveld2002}  However, in 2005 a synthesis of \tBuxantphos{} from 4,5-dilithio-9,9-(dimethyl)xanthene was reported.\cite{Mispelaere2005}  This synthesis involves lithiation of the xanthene backbone using \emph{n}-butyllithium and \gls{TMEDA} in heptane, followed by addition of \ce{ClP^{t}Bu2} and heating at 60 \degC{} for 24 hours, generating the product in 38\% isolated yield (Scheme \ref{scheme:Otherligandsynthesis}).
...
\begin{table}[htb]
\small
\caption[Crystallographic and natural bite-angles for diphosphine ligands]{Crystallographic\cite{Allen2002} and natural bite-angles\cite{Birkholz2009, Dierkes1999, Leeuwen2000, Aguila2009} for diphosphine ligands.} 
...
\begin{table}[htbp]
\caption[Selected NMR data for [(\tBuxantphos)H{]}\ce{CH(SO2CF3)2}]{Selected NMR data for the [(\tBuxantphos)H{]}\ce{CH(SO2CF3)2} compounds in \ce{CDCl3} ($\Delta\delta$ = $\delta$\sub{\ce{[(diphosphine)H]+}} - $\delta$\sub{diphosphine}).}
...
\begin{tabular}{l c c c}
	...
	\bfseries{Diphosphine}&\bfseries{$\delta/$ppm}&\bfseries{$\Delta\delta/$ppm}& \bfseries{\ce{H+}$\delta/$ppm}\\
	\midrule
...
\caption[Selected \phosphorus{} NMR data for \tBuxantphos{} selenides]{Selected \phosphorus{} NMR data for \tBuxantphos{} selenides in 1:1 \ce{CDCl3:CD2Cl2} ($\Delta\delta$ = $\delta$\sub{phosphine selenide} - $\delta$\sub{diphosphine}).}
\vspace{1em}
...
\begin{tabular}{l c c c c c c}
	\toprule
	~ & \multicolumn{2}{c}{\bfseries{P}} & \multicolumn{4}{c}{\bfseries{P=Se}} \\
	\cmidrule(lr){2-3} \cmidrule(lr){4-7}
	\bfseries{Diphosphine} & \bfseries{$\delta/$ppm} & \bfseries{$\Delta\delta/$ppm} & \bfseries{$\delta/$ppm} &  \bfseries{$\Delta\delta/$ppm} & \bfseries{\JPSe$/$Hz} & \bfseries{\pKb} \\
